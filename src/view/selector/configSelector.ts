...
export default async function ConfigSelector(cxt: ExtensionContext) {
  ...
  async function inputSource(
    ...
    const prompt = isLocal
      ...
      : "请输入远程路径，文件名后缀为.json或.yaml，如 https://xxx.com/file.json"
    ...
    state.sourcePath = await input.showInputBox({
      ...
      value: defaultValue,
      prompt,
      validate: isLocal ? isLegalPath : isLegalUrl, // TODO: 校验
  ...
  async function isLegalUrl(str: string) {
    const isLegal = /^((https|http)?:\/\/)[^\s]+(\.json|\.yaml)$/.test(str)
    return isLegal ? undefined : "url must end with .json or .yaml"
  }

  async function isLegalPath(str: string) {
    const isLegal = /^(\/\.*\w+)+/.test(str)
    return isLegal ? undefined : "illegal path"
  }
